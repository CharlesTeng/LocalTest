...
void task_aj(os_event_t *e){
//    char* lua_argv[] = { (char *)"lua", (char *)"-i", NULL };
//    NODE_DBG("Task task_lua started.\n");
    switch(e->sig){
        case SIG_AJ:
            AJ_Printf("SIG_AJ received.\n");
            AJ_Main();
            break;
        default:
            break;
...
void task_init_aj(void){	
	AJ_Printf("Start up the AJ Task !\n");
	taskQueue_aj = (os_event_t *)os_malloc(sizeof(os_event_t) * TASK_QUEUE_LEN);
	system_os_task(task_aj, USER_TASK_PRIO_1, taskQueue_aj, TASK_QUEUE_LEN);
}
...
void user_init(void)
{
	...
	task_init_aj();
    system_os_post(USER_TASK_PRIO_1,SIG_AJ,'s');
